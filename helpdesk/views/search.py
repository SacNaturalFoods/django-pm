...
class TabularSearchView(SearchView):
    ...
    def create_response(self, request):
        table = TicketTable([{
            ...
            'milestone':result.object.milestone.name if result.object.milestone else None,
...
def autocomplete_search(request):
    ...
    return HttpResponse(json.dumps(
        ...
        + [{'label': 'submitted_by', 'value': user.email} for user in users]
        + [{'label': 'not submitted_by', 'value': user.email} for user in users]
